---
title: "Resveratrol"
slug: "resveratrol"
aliases: ["Trans-Resveratrol", "Pterostilben", "Resveratrol Supplement"]
category: "antioxidants"
summary: "Resveratrol wurde als Rotwein-Inhaltsstoff bekannt und von David Sinclair popularisiert. Tierstudien sind beeindruckend — Humanstudien haben die Erwartungen bisher nicht erfüllt."
evidenceLevel: "3"
evidenceSummary: "Tierstudien zeigen eindrucksvolle lebensverlängernde Effekte. Humanstudien sind enttäuschend: Mehrere RCTs zeigten keinen klaren Nutzen bei gesunden Erwachsenen. Bei spezifischen Gruppen (Diabetes, Herzerkrankung) gibt es einzelne positive Signale, aber keine konsistenten Ergebnisse."
efsa_health_claims_allowed: false
efsa_notes: "EFSA hat keinen einzigen Health Claim für Resveratrol zugelassen. Entsprechende Anträge wurden abgelehnt mangels ausreichender Humanevidence. Produkte dürfen in der EU keine gesundheitsbezogenen Aussagen über Resveratrol machen."
safety_rating: "likely-safe"
typical_dose_mg: 500
publishedAt: 2026-05-14
key_studies:
  - title: "Resveratrol for Primary Prevention of Atherosclerosis: A Meta-Analysis"
    authors: "Sahebkar A et al."
    year: 2013
    pmid: "23861857"
    url: "https://pubmed.ncbi.nlm.nih.gov/23861857/"
    finding: "Meta-Analyse: Resveratrol senkte Triglyceride und LDL leicht, aber Effekte waren inkonsistent und klinisch wenig relevant."
  - title: "Resveratrol Levels and All-Cause Mortality in Older Community-Dwelling Adults (InCHIANTI)"
    authors: "Semba RD et al."
    year: 2014
    pmid: "24566624"
    url: "https://pubmed.ncbi.nlm.nih.gov/24566624/"
    finding: "Überraschend: Höhere Resveratrol-Spiegel im Urin (aus Nahrung) waren NICHT mit niedrigerer Sterblichkeit assoziiert — entgegen der Hypothese."
  - title: "Resveratrol Blunts the Positive Effects of Exercise Training on Cardiovascular Health"
    authors: "Gliemann L et al."
    year: 2013
    pmid: "23904096"
    url: "https://pubmed.ncbi.nlm.nih.gov/23904096/"
    finding: "Resveratrol-Supplementierung hemmte bei älteren Männern die positiven kardiovaskulären Trainingseffekte — ein unerwarteter negativer Befund."
---

## Das Versprechen und die Realität

Resveratrol wurde durch David Sinclairs Forschung und das populäre Buch „Lifespan" berühmt. Das Molekül aktiviert in Laborversuchen Sirtuine (SIRT1) — Proteine, die mit Langlebigkeit assoziiert werden. Bei Mäusen verlängerte es die Lebensspanne deutlich.

Das Problem: Beim Menschen ist der Transfer bisher nicht gelungen.

## Was sagen die Humanstudien?

### Das Positive
- Einige kleine Studien zeigen anti-entzündliche Effekte
- Bei Diabetikern: einzelne positive Signale für Blutzucker und Insulinsensitivität
- Relativ sicheres Sicherheitsprofil

### Das Problem
- Die InCHIANTI-Studie (eine der wenigen echten Bevölkerungsstudien) fand keinen Zusammenhang zwischen Resveratrol-Spiegeln und Sterblichkeit
- Resveratrol hemmt nachweislich positive Trainingseffekte — ein kontraintuitiver Befund
- Bioverfügbarkeit ist niedrig: Der Körper baut Resveratrol sehr schnell ab
- Kein einziger RCT hat lebensverlängernde Effekte beim Menschen gezeigt

## Warum das Rotwein-Argument nicht zieht

Rotwein enthält nur ~1–2 mg Resveratrol pro Glas. Supplements enthalten 250–1.000 mg. Die Alkohol-Toxizität des Weins macht den minimalen Resveratrol-Gehalt mehr als wett — Rotwein trinken „wegen Resveratrol" ist nutritionswissenschaftlich nicht haltbar.

## Sicherheit

Bei üblichen Dosen (bis 1.000 mg/Tag) gut verträglich. Bei sehr hohen Dosen (2.000+ mg/Tag) wurden Magenprobleme berichtet. Wechselwirkungen mit Blutverdünnern möglich (Resveratrol hemmt Thrombozyten).

## Unser Fazit

Resveratrol ist wissenschaftlich interessant, aber als Supplement derzeit nicht empfehlenswert — die Humanstudien rechtfertigen weder den Preis noch die Erwartungen aus dem Marketing.
